...
In the *Run Transformation* interface, select the B specification you want to apply to all pivot files of the project.  
In this example, we choose project `T0_MainTests` and apply the machine `T0_MainTransformation.mch`.

Select **Parallel** if you want the execution of the transformation to be done in parallel for the test files.  
This launches the runner in separate jobs, which enhances performance compared with sequential execution.

#### Transformation Runner

The *Transformation Runner* is then launched. It shows the parallel jobs and their execution state.  
When finished, you obtain a summary of the execution.
...
These metrics highlight the benefit of parallel execution compared to a sequential run.

#### Trace View

Open the *Trace View* by clicking on the corresponding icon in the *Transformation Runner*.

The Trace view shows the files on which the transformation has been applied.

Click on **Expand all** to see the operations that have been applied for every file.

You can inspect these files to check whether the result is the expected one.

#### Notes

- Each pivot file corresponds to one test case.
- The operations listed in the trace correspond to the rules triggered during the transformation.
- The state of `0_empty.pivot` is **UNKNOWN**, since it is empty and no transformation rule has been triggered.